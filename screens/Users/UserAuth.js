...
import { changeUserMode, hideError, userAuthLogin, userAuthRegister } from '../../store/authAction/authAction'
...
const UserAUth= props=>{
    ...
    const handleSubmitForm= (email, password, fullName, phoneNumber, gender)=>{
        if(signup){
            ...
            setError(false)
            dispatch(userAuthRegister(email,password,fullName,phoneNumber,gender))
        }else{
            setTryAuth(true)
            dispatch(userAuthLogin(email, password))
            setTimeout(()=>{
                setTryAuth(false)
            },1000)
